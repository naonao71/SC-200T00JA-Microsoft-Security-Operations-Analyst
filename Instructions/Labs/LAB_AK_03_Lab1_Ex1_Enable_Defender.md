...
10. 「メンバー」タブで、「＋メンバーを追加する」をクリックし、「**MOD Administrator**」を選択して、「選択」ボタンをクリックします。
11. 「レビューと割り当て」を2回クリックして、所有者ロールを管理者アカウントに割り当てます。
...
14. 「データソースの追加」を選択し、「確認と作成」を選択します。
15. 検証に成功した後に、「作成」を選択します。
16. **データ収集ルール** を作成すると WINServer への AzureMonitorWindowsAgent 拡張機能のインストールが開始されます。
17. データ収集ルールの作成が完了したら、Azure ARCの「WINServer」を確認します。
18. 「設定」セクションの「拡張機能」を選択します。
19. **AzureMonitorWindowsAgent** が「成功」の状態で一覧表示されます。
20. 次のラボに進み、しばらくたってから Microsoft Defender for Cloud の [インベントリ] セクションを確認し、WINServer が含まれていることを確認できます。
